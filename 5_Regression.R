...
m<-lm(log_saidi~
        TUVU+MODO+HOSP+OSPR+RTHA+RWBL+RBWO+PIWO+HAWO+NOFL+EUST+AMCR+MOPA+
        month+year+
...
m.s<-lm(log_saidi~TUVU+MODO+HOSP+OSPR+RTHA+RWBL+RBWO+PIWO+HAWO+NOFL+EUST+AMCR+MOPA,data=dp)
m.s2<-lmer(log_saidi~TUVU+MODO+HOSP+OSPR+RTHA+RWBL+RBWO+PIWO+HAWO+NOFL+EUST+AMCR+MOPA+(1|actual_city_town),
         data = dp)
...
m.st2<-lmer(log_saidi~(TUVU*month)+(MODO*month)+(HOSP*month)+(OSPR*month)+(RTHA*month)+(RWBL*month)+
           (PIWO*month)+(HAWO*month)+(NOFL*month)+(EUST*month)+year+(1|actual_city_town),data=dp)
...
#MODO,RTHA,PIWO,EUST,MOPA have significant habitat interaction
...
m.sth<-lm(log_saidi~TUVU+(MODO*month)+(MODO*Forest)+(HOSP*month)+OSPR+(RTHA*Forest)+
  (RWBL*month)+(PIWO*Forest)+(HAWO*month)+NOFL+(EUST*Forest)+AMCR+(MOPA*Forest)+year+
    Barren_Land,data=dp)
m.sth2<-lmer(log_saidi~TUVU+(MODO*month)+(MODO*Forest)+(HOSP*month)+OSPR+(RTHA*Forest)+
            (RWBL*month)+(PIWO*Forest)+(HAWO*month)+NOFL+(EUST*Forest)+AMCR+(MOPA*Forest)+year+
            Barren_Land+(1|actual_city_town),data=dp)
...
models <- list(m.s, m.t, m.h, m.st, m.sh, m.sth,
               m.th)
models2 <- list(m.s2, m.t2, m.h2, m.st2, m.sh2, m.sth2,
               m.th2)
...
aic<-aictab(cand.set = models2, modnames = mod.names)

#Compare model performance in a table
df<-as.data.frame(matrix(ncol = 5 , nrow= 7))
colnames(df)<- c("Model","F.Statistic","P","RSE","R.Squared")
df$Model<-mod.names
df$F.Statistic<-lapply(models2, summary)%>%
  sapply('[', "fstatistic")%>%
  sapply('[',"value")
df$R.Squared<-lapply(models2, summary)%>%
  sapply('[', "r.squared")%>%
  unlist()
df$RSE<-lapply(models2, summary)%>%
  sapply('[', "sigma")%>%
  unlist()
df$P<-sapply(models2, function(x){
...
write.csv(df,"Outputs/predictor_selection_models_updated.csv",row.names = F)
...
#PIWO and MOPA occupy rural areas vs HOSP and NOFL occupying urban areas
m.resident<-lm(log_saidi~(RWBL*month)+OSPR+year+
            Barren_Land,data=dp)
m.resident2<-lmer(log_saidi~(RWBL*month)+OSPR+year+
                 Barren_Land+(1|actual_city_town),data=dp)
m.migrant<-lm(log_saidi~(RTHA*Forest)+(HAWO*month)+year+
                Barren_Land,data=dp)
m.migrant2<-lmer(log_saidi~(RTHA*Forest)+(HAWO*month)+year+
                Barren_Land+(1|actual_city_town),data=dp)
m.rural<-lm(log_saidi~(MOPA*Forest)+(PIWO*Forest)+year+
            Barren_Land,data=dp)
m.rural2<-lmer(log_saidi~(MOPA*Forest)+(PIWO*Forest)+year+
              Barren_Land+(1|actual_city_town),data=dp)
...
write.csv(df.pc,"Outputs/species_subset_models_updated.csv",row.names = F)
...
m.summer<-lm(log_saidi~TUVU+(MODO*Forest)+HOSP+OSPR+(RTHA*Forest)+
            RWBL+(PIWO*Forest)+HAWO+NOFL+(EUST*Forest)+AMCR+(MOPA*Forest)+year+
            Barren_Land,data=dp%>%filter(season=="summer"))
m.summer2<-lmer(log_saidi~TUVU+(MODO*Forest)+HOSP+OSPR+(RTHA*Forest)+
               RWBL+(PIWO*Forest)+HAWO+NOFL+(EUST*Forest)+AMCR+(MOPA*Forest)+year+
...
m.winter2<-lmer(log_saidi~TUVU+(MODO*Forest)+HOSP+OSPR+(RTHA*Forest)+
               RWBL+(PIWO*Forest)+HAWO+NOFL+(EUST*Forest)+AMCR+(MOPA*Forest)+year+
               Barren_Land+(1|actual_city_town),data=dp%>%filter(season=="winter"))
m.Forest<-lm(log_saidi~TUVU+(MODO*month)+(HOSP*month)+OSPR+RTHA+
               (RWBL*month)+PIWO+(HAWO*month)+NOFL+EUST+AMCR+MOPA+year+
               Barren_Land,data=dp%>%filter(Forest>quantile(Forest,0.75)))
m.Forest2<-lmer(log_saidi~TUVU+(MODO*month)+(HOSP*month)+OSPR+RTHA+
               (RWBL*month)+PIWO+(HAWO*month)+NOFL+EUST+AMCR+MOPA+year+
...
m.Developed2<-lmer(log_saidi~TUVU+(MODO*month)+(HOSP*month)+OSPR+RTHA+
                  (RWBL*month)+PIWO+(HAWO*month)+NOFL+EUST+AMCR+MOPA+year+
...
models2 <- list(m.sth2,m.summer2,m.winter2,m.Forest2, m.Developed2)
...
write.csv(df.patterns,"Outputs/space_time_subset_models_updated.csv",row.names = F)

#Make table of species coefficients in each model
sp_list<-c("TUVU","MODO","HOSP","OSPR","RTHA",
             "RWBL","PIWO","HAWO","NOFL","EUST","AMCR","MOPA")
